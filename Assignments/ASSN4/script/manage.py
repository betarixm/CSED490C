...
class DatasetGenerator(Executable):
    def generated_dataset(self, cwd: Path | None = None) -> Dataset:
        ...
        results = [
            (
                path.split("/")[-1],
                Path(path) / "input0.ppm",
                Path(path) / "input1.raw",
                Path(path) / "output.ppm",
            )
            for path in glob.glob(f"{cwd}/Convolution/Dataset/*")
        ]
...
class Target(Executable):
    def run(self, data: Data, cwd: Path | None = None):
        ...
        result = self(
            ["-e", output, "-i", f"{input0},{input1}", "-t", "image"],
            cwd=cwd,
            capture_output=True,
        )
...
class Project:
    ...
    def get_target_pathes(self):
        return [
            Path(path)
            for path in glob.glob(str(self._source_dir / "Convolution_template*"))
        ]
    ...
    def get_target_path(self):
        return Path(self._source_dir / f"Convolution_template")
