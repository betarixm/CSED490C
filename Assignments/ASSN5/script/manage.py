...
class DatasetGenerator(Executable):
    def generated_dataset(self, cwd: Path | None = None) -> Dataset:
        ...
        results = [
            ...
            for path in glob.glob(f"{cwd}/Histogram/Dataset/*")
        ]
...
class Target(Executable):
    def run(self, data: Data, cwd: Path | None = None):
        ...
        result = self(
            ["-e", output, "-i", f"{input0}", "-t", "integral_vector"],
            cwd=cwd,
            capture_output=True,
        )
...
class Project:
    ...
    def get_target_pathes(self):
        return [
            Path(path)
            for path in glob.glob(str(self._source_dir / "Histogram_template*"))
        ]
    ...
    def get_target_path(self):
        return Path(self._source_dir / f"Histogram_template")
