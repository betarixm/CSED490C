...
class Project:
    ...
    def make_target(
        self, number_of_streams: int = 16, suppress_compile: bool = False
    ) -> Target:
        target_path = self.get_target_path_by_number_of_streams(number_of_streams)

        if not suppress_compile:
            template = open(self._source_dir / "template.cu", "r").read()

            source = template.replace(
                "#define NUM_STREAMS 16", f"#define NUM_STREAMS {number_of_streams}"
            )
            ...
            try:
                ...
                os.rename(
                    self._source_dir / "StreamVectorAdd_template",
                    target_path,
                )
            finally:
                writer = open(self._source_dir / "template.cu", "w")
    ...
    def get_target_pathes(self):
        return [
            Path(path)
            for path in glob.glob(str(self._source_dir / "StreamVectorAdd_template*"))
        ]

    def get_targets(self):
        return [Target(target_path) for target_path in self.get_target_pathes()]

    def get_target_path_by_number_of_streams(self, number_of_streams: int):
        return Path(self._source_dir / f"StreamVectorAdd_template_{number_of_streams}")

    def get_target_by_number_of_streams(self, number_of_streams: int):
        return Target(self.get_target_path_by_number_of_streams(number_of_streams))
...
def _evaluate(project: Project, cwd: Path | None = None):
    ...
    results_by_data = _run_target_with_dataset(
        project.get_target_by_number_of_streams(16), dataset
    )

    results_by_number_of_streams = _run_targets_with_data(project.get_targets(), dataset[-1])

    results_by_number_of_streams = [
        Result(
            key=re.findall(r"(\d+)", result.key)[0],
            is_correct=result.is_correct,
            logs=result.logs,
        )
        for result in results_by_number_of_streams
    ]

    results_by_number_of_streams = sorted(results_by_number_of_streams, key=lambda x: int(x.key))
    ...
    return template.render(
        ...
        results_by_number_of_streams_fields=[
            "Number of Streams",
            "Correctness",
            *[log.description for log in results_by_number_of_streams[0].logs],
        ],
        results_by_number_of_streams_values=[
            (
                result.key,
                result.is_correct,
                *(str(log.elapsed_time_in_ms) + " ms" for log in result.logs),
...
def _compile_project(project: Project):
    ...
    for index_of_streams in range(32):
        project.make_target(number_of_streams=index_of_streams + 1)
