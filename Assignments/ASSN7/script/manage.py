...
class DatasetGenerator(Executable):
    def generated_dataset(self, cwd: Path | None = None) -> Dataset:
        ...
        results = [
            ...
            for path in glob.glob(f"{cwd}/SparseMV/Dataset/*")
        ]
...
class Project:
    ...
    def make_targets(self) -> Tuple[Target, Target]:
        f = open(self._source_dir / "template.cu")
        code = f.read()
        f.close()

        f = open(self._source_dir / "template.cu", "w")
        f.write(f"#define USING_SHARED_MEMORY 1\n{code}")
        f.close()

        subprocess.run(["make", "template"], cwd=self._source_dir)

        shutil.move(self.get_target_path(), self.get_target_path(True))
        ...
        return Target(self.get_target_path()), Target(
            self.get_target_path(True)
        )

    def make_clean(self):
        subprocess.run(["make", "clean"], cwd=self._source_dir)

    def get_target_path(self, using_shared_memory: bool = False):
        if using_shared_memory:
            return Path(self._source_dir / f"JDS_T_template_shared_memory")
        else:
            return Path(self._source_dir / f"JDS_T_template")
...
def _evaluate(project: Project, cwd: Path | None = None):
    ...
    target, target_using_shared_memory = project.make_targets()

    results_by_data = _run_target_with_dataset(target, dataset)

    results_by_data_using_shared_memory = _run_target_with_dataset(
        target_using_shared_memory, dataset
    )
    ...
    return template.render(
        ...
        results_by_data_fields_using_shared_memory=[
            "Data",
            "Correctness",
            *[log.description for log in results_by_data_using_shared_memory[0].logs],
        ],
        results_by_data_values_using_shared_memory=[
            (
                result.key,
                result.is_correct,
                *(str(log.elapsed_time_in_ms) + " ms" for log in result.logs),
            )
            for result in results_by_data_using_shared_memory
        ],
    )
